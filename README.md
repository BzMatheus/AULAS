# 📚 Atividades e Entregas Acadêmicas

Este repositório serve como um arquivo central para organizar e armazenar todas as atividades, listas de exercícios, trabalhos práticos e relatórios que desenvolvi para entregar aos professores ao longo da faculdade. 

O objetivo principal é manter meu histórico acadêmico organizado e demonstrar minha evolução nas disciplinas do curso.

---

## 📅 Entregas por Período / Semestre

### 🏛️ [1º Semestre - 2026]

* **Algoritmo e Lógica de Programação** (Prof. Gisele)
  * 📄 [Documentação Gerador de Senha](<Algoritmo e Lógica de Programação/Documentação de GeradordeSenha.docx>) - Resolução de exercícios práticos sobre [Tema].
* **Sistemas Operacionais** (Prof. Ponce)
  * 📊 [Trabalho Supercomputador FUGAKU](<./Sistemas Operacional/TRABALHO DE SISTEMAS OPERACIONAIS.pdf>) - Trabalho sobre o Supercomputador FUGAKU
  * 📝 [Lista de Exercicio 3](<./Sistemas Operacional/lista de exercicio 3.docx>) - Exercicios Linux.
  * 📝 [Lista de Exercicio 4](<./Sistemas Operacional/Lista de exercicios SO 4 (2).docx>) - Exercicios Linux.
* **Projeto Integrador** (Prof. Julio)
  * 📄 [Documentação Linguagem e FrameWork](<Projeto Integrador/Linguagens e Frameworks.docx>) - Documento sobre Linguagens e FrameWork.
  * 📄 [Documentação Análise de Sistemas de Questionarios](<Projeto Integrador/Atividade Pesquisa e Análise de Sistemas de Questionários Existentes (edição final).docx>) - Análise de Sistemas de Questionarios Existentes.
  * 📄 [Documentação Pitch](<Projeto Integrador/Pitch Sistema PROJ INT (1).pptx>) - Pitch Sistema PROJ INT
  * 📄 [Documentação Visão de Escopo](<Projeto Integrador/Visão de Escopo Sistema Questionário.docx>) - Visão de Escopo Sistema Questionário
  * 📄 [Documentação Backlog](<Projeto Integrador/BackLogAtt (2).docx>) - Backlog
* **Arquitetura e Organização Computacional** (Prof. Maciel)
  * 📊 [Seminario Sistema Embarcados/Computação Ubíqua](<./Arquitetura e Organização Computacional/Seminário de Arquitetura e Organização de Computadores_compressed.pdf>) - Sistema Embarcado/Computação Ubíqua
---


